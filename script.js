...
async function completeRoute() {
    ...
    const pilotId = document.getElementById('pilotId').value.trim();
    
    if (!pilotName || !pilotId) {
        showStatus('Please enter pilot name and ID', 'error');
        return;
    }
    
    try {
        await sendDiscordMessage('completed', pilotName, pilotId);
...
async function cancelRoute() {
    ...
    try {
        await sendDiscordMessage('cancelled', pilotName, pilotId);
        showStatus('Route completed successfully!', 'success');
...
async function sendDiscordMessage(action, pilotName, pilotId) {
    ...
    if (action === 'completed') {
        ...
        description = `${pilotName} (ID: ${pilotId}) has successfully completed route ${route.id}`;
    } else if (action === 'cancelled') {
        color = 0xEF4444; // Red
        title = '❌ Route Cancelled';
        description = `${pilotName} (ID: ${pilotId}) has cancelled route ${route.id}`;
    }
...
function loadPilotInfo() {
    ...
    const savedId = localStorage.getItem('amerifed_pilot_id');
    
    if (savedName) document.getElementById('pilotName').value = savedName;
    if (savedId) document.getElementById('pilotId').value = savedId;
    ...
    document.getElementById('pilotId').addEventListener('input', function() {
        localStorage.setItem('amerifed_pilot_id', this.value);
    });
}
